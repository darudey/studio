...
export const getNotificationsForUser = async (userId: string): Promise<Notification[]> => {
    try {
        ...
        if (error instanceof Error && error.message.includes("Missing or insufficient permissions")) {
            ...
            console.error("Firestore Index Required: Please create a composite index for the 'notifications' collection on (userId, createdAt). The link to create it should be in your browser's developer console.", error);
        } else {
...
export const markUserNotificationsAsRead = async (userId: string): Promise<void> => {
    try {
        ...
        if (error instanceof Error && error.message.includes("The query requires an index")) {
             console.error("Firestore Index Required: Cannot mark notifications as read without a composite index on (userId, isRead). The link to create it should be in your browser's developer console.", error);
        } else {
